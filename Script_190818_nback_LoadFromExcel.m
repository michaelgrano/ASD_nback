...
for sense = 0:1
    ...
    for n = subjects(1:length(subjects))
        ...
        if sense == 0
            ...
            save(strcat('MatDataSub',num2str(n),'190818.mat'),'thedata'); %save data
        else
            %if there are stimuli
            save(strcat('StimMatDataSub',num2str(n),'190818.mat'),'thedata');
        end
...
function [time,x,psize] = moveOver(cspreadsheet,sheet)
    ...
    time = time_all(design_start:size(time_all,1),1); %truncated column 1 data to start of block
    x = x_all(design_start:size(x_all,1),1); %truncated column 2 data to start of block
    psize = psize_all(design_start:size(psize_all,1),1); %truncated column 4 data to start of block
